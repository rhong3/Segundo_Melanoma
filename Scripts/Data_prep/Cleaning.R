...
histology = read_excel("~/Documents/Segundo_Melanoma/Data/Sumary Histology evaluation, Segundo study.xlsx")
...
his_clin$survival_3yr="NA"
his_clin$survival_3yr[which(his_clin$dss.days>=1095)] = 1
his_clin$survival_3yr[which(his_clin$dss.days<1095 & his_clin$dss.events==1)] = 0

his_clin$survival_1yr="NA"
his_clin$survival_1yr[which(his_clin$dss.days>=365)] = 1
his_clin$survival_1yr[which(his_clin$dss.days<365 & his_clin$dss.events==1)] = 0

his_clin$survival_6mo="NA"
his_clin$survival_6mo[which(his_clin$dss.days>=183)] = 1
his_clin$survival_6mo[which(his_clin$dss.days<183 & his_clin$dss.events==1)] = 0
write.csv(his_clin, "~/Documents/Segundo_Melanoma/Data/clinical&histology.csv")


# Proteomics
prot <- read_delim("~/Documents/Segundo_Melanoma/Data/proteomics/20191211_Segundo_MM_TMT11_isotope_correction_normalized_ratios_Final.txt", "\t", escape_double = FALSE, trim_ws = TRUE, skip = 3)
...
tumor <- read_excel("~/Documents/Segundo_Melanoma/Data/Sample tumor content.xlsx")
...
prot_his_clin <- read.csv("~/Documents/Segundo_Melanoma/Data/clinical&histology.csv", row.names=1)
...
write.csv(prot.sample, "~/Documents/Segundo_Melanoma/Data/proteomics/proteomics.csv", row.names = FALSE)
prot.sample.ICA = select(prot.sample, -c("Gene name", "Description", "Accession"))
rownames(prot.sample.ICA) = prot.sample$Accession
write.csv(prot.sample.ICA, "~/Documents/Segundo_Melanoma/Data/proteomics/ICA_proteomics.csv", row.names = TRUE)
prot_his_clin = prot_his_clin[intersect(colnames(prot.sample), rownames(prot_his_clin)), ]
write.csv(prot_his_clin, "~/Documents/Segundo_Melanoma/Data/proteomics/proteomics_clinical.csv")


# Transcriptomics
Transcriptomics = read_excel("~/Documents/Segundo_Melanoma/Data/transcriptomics/Transcriptomics_final.214.allMm.not.meancentered_rounded.xlsx")
...
Trans_his_clin <- read.csv("~/Documents/Segundo_Melanoma/Data/clinical&histology.csv", row.names=1)
...
write.csv(Trans.sample, "~/Documents/Segundo_Melanoma/Data/transcriptomics/transcriptomics.csv", row.names = FALSE)
Trans.sample.ICA = select(Trans.sample, -c("Gene name", "Description"))
rownames(Trans.sample.ICA) = Trans.sample$`Gene name`
write.csv(Trans.sample.ICA, "~/Documents/Segundo_Melanoma/Data/transcriptomics/ICA_transcriptomics.csv", row.names = TRUE)
Trans_his_clin = Trans_his_clin[intersect(colnames(Trans.sample), rownames(Trans_his_clin)), ]
write.csv(Trans_his_clin, "~/Documents/Segundo_Melanoma/Data/transcriptomics/transcriptomics_clinical.csv")


# Phospho
# filepath is path to 'PhosphoDIA_peptides_MM_Segundo_Cohort_NORMALIZED.txt'
...
write.csv(Phospho.sample, "~/Documents/Segundo_Melanoma/Data/phospho/phospho.csv", row.names = FALSE)
...
write.csv(Phospho.sample.ICA, "~/Documents/Segundo_Melanoma/Data/phospho/ICA_phospho.csv", row.names = TRUE)
Phospho_his_clin = Phospho_his_clin[intersect(colnames(Phospho.sample), rownames(Phospho_his_clin)), ]
write.csv(Phospho_his_clin, "~/Documents/Segundo_Melanoma/Data/phospho/phospho_clinical.csv")
